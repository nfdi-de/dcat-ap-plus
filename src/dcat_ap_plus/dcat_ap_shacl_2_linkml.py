...
DATATYPES = ["dateTime", "decimal", "duration", "hexBinary", "nonNegativeInteger"]
...
def parse_dcat_ap_shacl_shapes(builder):
    ...
    for node_shape in dcat_ap_shapes["shapes"]:
        ...
        if node_curie == "dcat:Resource":
            ...
            node_name = (
                node_shape["@id"].split("#")[-1].split(":")[-1].replace("Shape", "")
            )
        ...
        if node_name not in DATATYPES + IGNORED_NODES:
            ...
            # Add DCAT-AP main classes
                ...
                builder.add_class(
                    ClassDefinition(
                        name=node_name, class_uri=node_curie, description=description
                    )
                )
            ...
            if node_shape["sh:property"]:
                for slot_shape in node_shape["sh:property"]:
                    ...
                    required = (
                        True
                        if "sh:minCount" in slot_shape
                        and int(slot_shape["sh:minCount"]) == 1
                        else False
                    )
                    multivalued = (
                        False
                        if "sh:maxCount" in slot_shape
                        and int(slot_shape["sh:maxCount"]) == 1
                        else True
                    )
                    ...
                    if "sh:class" in slot_shape:
                        ...
                        if get_curie(slot_shape["sh:class"]) == "dcat:Resource":
                            ...
                            slot_range = get_curie(slot_shape["sh:class"]).split(":")[
                                -1
                            ]
                    ...
                    # Add the class slot
                        ...
                        class_slots[slot_name] = SlotDefinition(
                            name=slot_name,
                            slot_uri=slot_curie,
                            description=description,
                            required=required,
                    ...
                    builder.schema.classes[node_name].slots = sorted(
                        list(class_slots.keys())
                    )
                    builder.schema.classes[node_name].slot_usage = {
                        key: class_slots[key] for key in sorted(class_slots)
                    }
...
def build_dcatap_linkml():
    ...
    builder.schema.prefixes["dcatap_linkml"] = (
        "https://w3id.org/nfdi-de/dcat-ap-linkml/"
    )
    ...
    builder.schema.classes = {
        key: builder.schema.classes[key] for key in sorted(builder.schema.classes)
    }
    builder.schema.slots = {
        key: builder.schema.slots[key] for key in sorted(builder.schema.slots)
    }
    builder.schema.types = {
        key: builder.schema.types[key] for key in sorted(builder.schema.types)
    }

    # TODO list
    builder.schema.todos = [
        'Think about how to add all the other enums and their permissible values to constrain the allowed instances of classes such as "Concept", "MediaType", etc. as defined in https://semiceu.github.io/DCAT-AP/releases/3.0.0/#controlled-vocs. Using EnumBindings (https://linkml.io/linkml-model/latest/docs/bindings/) seems best, but does not yet work.',
        "Check if https://github.com/linkml/linkml/issues/1813 is closed and range unions are validatable",
    ]
...
def build_dcatap_plus():
    ...
    def extend_dataset():
        ...
        slots = ["id", "is_about_entity", "is_about_activity"]
    ...
    def extend_activity():
        ...
        activity.slot_usage = {
            ...
            "has_qualitative_attribute": {"notes": ["not in DCAT-AP"]},
            "has_quantitative_attribute": {"notes": ["not in DCAT-AP"]},
            "had_input_entity": {"notes": ["not in DCAT-AP"]},
            "had_output_entity": {"notes": ["not in DCAT-AP"]},
            "had_input_activity": {"notes": ["not in DCAT-AP"]},
            "carried_out_by": {"notes": ["not in DCAT-AP"]},
        }
        activity.mixins = ["ClassifierMixin"]
        activity.in_subset = ["domain_agnostic_core"]
        activity.notes = [
            "The specified properties (slots) of this class are part of our extension of the DCAT-AP."
        ]
    ...
    def extend_supportive_entities():
        for supportive_entity in builder.schema.classes.keys():
            ...
            if supportive_entity not in (
                MAIN_NODES + DATATYPES + ["DataGeneratingActivity"]
            ):
                if supportive_entity in [
                    "Resource",
                    "Document",
                    "LegalResource",
                    "LicenseDocument",
                ]:
                    builder.schema.classes[supportive_entity].slots = slots + [
                        "id",
                        "title",
                        "description",
                    ]
    ...
    def add_classification_context():
        builder.add_class(
            ClassDefinition(
                ...
                abstract=True,
                slots=["type", "rdf_type"],
                slot_usage={"type": {"inlined": True, "range": "DefinedTerm"}},
                in_subset="domain_agnostic_core",
            )
        )
        builder.add_class(
            ClassDefinition(
                ...
                slots=["id", "title"],
                slot_usage={"title": {"slot_uri": "schema:name"}},
                attributes={
                    "from_CV": {
    ...
    def add_attribute_context():
        ...
        todos = [
            "Dynamic enums (https://linkml.io/linkml/schemas/enums.html#dynamic-enums) should be used to "
